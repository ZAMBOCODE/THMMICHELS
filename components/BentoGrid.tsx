        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 auto-rows-[300px]">
          {cards.map((card) => (
            <Link
              key={card.href}
              to={card.href}
              className="relative overflow-hidden group border border-white/10 row-span-1 transition-all duration-300 hover:border-[#D97706]/50 scroll-animate-scale cursor-pointer block"
              style={{ transitionTimingFunction: 'var(--ease-smooth)' }}
              style={{ transitionDelay: card.delay }}
            >
              <img src={card.image} alt={card.alt} loading="lazy" className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" style={{ transitionTimingFunction: 'var(--ease-smooth)' }} />
              ...
              <div className="absolute inset-0 p-8 flex flex-col justify-end z-20">
                ...
                <p className="text-[#9CA3AF] text-sm max-w-xs transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 font-mono" style={{ transitionTimingFunction: 'var(--ease-smooth)' }}>
                  {card.desc}
                </p>
                <span className="mt-4 text-[#D97706] text-xs font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all duration-300 delay-75 flex items-center gap-2" style={{ transitionTimingFunction: 'var(--ease-smooth)' }}>Mehr erfahren <span className="text-lg">→</span></span>
              </div>
            </Link>
          ))}
        </div>
